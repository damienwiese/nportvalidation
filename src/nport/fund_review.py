...
GAP_GUIDE = [
    ("G-001", "OPEN INPUT", "Independent position population", "CLI + Sources + HoldingFields",
     ...
     "Register the exact independent position-export path on Sources; no separate review template"),
    ("G-002", "OPEN INPUT", "Independent fund-accounting close", "Sources + FundFields",
     ...
     "Register the actual GL/NAV close evidence on Sources; no prescribed filename"),
    ("G-003", "OPEN INPUT / PARTLY AUTOMATED", "Create/redeem flows", "CLI + Sources + FundFields",
     ...
     "Register the actual order/reinvestment evidence on Sources; no prescribed filename"),
    ("G-004", "OPEN INPUT", "Fund policy and static data", "Sources + FundFields",
     ...
     "Register the actual policy/static-data evidence on Sources; no prescribed filename"),
    ("G-005", "IMPLEMENTED CONTROL", "Source manifest", "Sources",
     ...
     "All remediation remains in filing_inputs.xlsx; build emits source_manifest.csv"),
    ("G-006", "ENFORCED CONTROL", "Prohibited comparison inputs", "Sources + source files",
     ...
     "No remediation evidence may come from comparison-only files"),
    ("G-007", "IMPLEMENTED CONTROL", "Manual input trace", "FundFields + HoldingFields + Sources",
     ...
     "All entries remain in filing_inputs.xlsx; build emits field_provenance.csv and input_receipt.json"),
    ("G-008", "CONDITIONAL OPEN INPUT", "Swap economics", "Sources + HoldingFields",
     ...
     "Register the actual internal trade/confirmation evidence on Sources; no prescribed filename"),
    ("G-009", "CONDITIONAL OPEN INPUT", "B.2.f / B.3 / B.5.c / B.9 / B.10", "Sources + FundFields",
     ...
     "Register each actual accounting/risk source on Sources; no prescribed filenames"),
    ("G-010", "CONDITIONAL OPEN INPUT", "C.7 liquidity", "Sources + HoldingFields",
     ...
     "Register the actual liquidity-risk evidence on Sources; no prescribed filename"),
    ("G-011", "CONDITIONAL OPEN INPUT", "Option terms and delta", "Sources + HoldingFields",
     ...
     "Register the actual option/risk evidence on Sources; no prescribed filename"),
    ("G-012", "IMPLEMENTED RECONCILIATION INPUT", "Independent reconciliation", "Sources + ReconciliationInputs; results in reconciliation.csv",
     ...
     "Register the actual independent control evidence on Sources; enter its totals only in ReconciliationInputs; build emits reconciliation.csv"),
    ("G-013", "TRUE TECHNICAL LIMITATION", "Authenticated origin assurance", "No workbook field closes this item",
     ...
     "Outside the active remediation workflow; no workbook entry"),
]

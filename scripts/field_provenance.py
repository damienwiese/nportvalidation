...
ROWS: list[tuple[str, str, str, str, str, str]] = [
    ...
    ("Option", "delta", HUMAN, "option_delta sheet; N/A is schema-valid and may be replaced with a supported report-date value", "master_sheet.py", r"deltas = review\.option_delta"),
    ...
    ("Fund policy", "fiscalYearEndMMDD / derivativesRegimePolicy / liquidityRequired / cashB2fRequired / policyEffectiveFrom / policyEffectiveTo", HUMAN,
     "fund_policy sheet; populated values are written to the matching fund_config.txt and filing context is derived", "humanreview.py", r"def apply_fund_policy_to_configs"),

    # ---- Newly supported conditional sections ------------------------------
    ("Conditional sections", "cashNotReportedInCOrD (B.2.f)", HUMAN,
     "fund_risk sheet; required only when fund_policy cashB2fRequired is Y",
     "filing_master.py", r"cashNotReportedInCOrD.*monthlyReturnCategoriesJson"),
    ("Conditional sections", "monthlyReturnCategoriesJson (B.5.c)", HUMAN,
     "fund_risk sheet; populated only when applicable and supported",
     "humanreview.py", r"monthlyReturnCategoriesJson"),
    ("Conditional sections", "derivativesRegime", "Derived",
     "Derived from the reviewed derivativesRegimePolicy for the fund and period",
     "policy.py", r"derivatives_regime=context\.policy\.derivatives_regime"),
    ("Conditional sections", "derivExposurePct / derivCurrencyExposurePct / derivInterestRateExposurePct / derivDaysInExcess (B.9)", HUMAN,
     "fund_risk sheet; required only when reviewed fund policy is LIMITED",
     "humanreview.py", r"if regime == \"LIMITED\""),
    ("Conditional sections", "medianDailyVarPct / medianVarRatioPct / backtestingExceptions (B.10)", HUMAN,
     "fund_risk sheet; required only for the applicable reviewed VaR regime",
     "humanreview.py", r"if regime in \{\"VAR_ABSOLUTE\", \"VAR_RELATIVE\"\}"),
    ("Conditional sections", "liquidityClassificationJson / liquidityCircumstancesJson (C.7)", HUMAN,
     "holding_liquidity sheet; classification is required only when reviewed policy requires C.7",
     "master_sheet.py", r"liquidity = review\.holding_liquidity"),
]
